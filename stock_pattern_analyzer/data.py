...
class DataHolder:
    def __init__(self, tickers: list, window_size: int, period_years: int = 5, interval: int = 1):
        ...
        self._max_values_per_stock = self.period_years * self.interval
        ...
        self.ticker_originals = np.zeros((len(self.tickers), self._max_values_per_stock), dtype=np.float)

        # This holds the normalized values of the selected window from a stock
        self.ticker_windows_norm = np.zeros((self._max_nb_windows, self.window_size), dtype=np.float)
    ...
    def _collect_data_for_ticker(self, ticker: str, label: int):
        row_index = 0

        # Df is reversed so the most recent values are at the top
        ticker_df = yfinance.Ticker(ticker).history(period=f"{self.period_years}y", interval=f"{self.interval}d")[::-1]
    ...
    def fill_data(self):
        for label, ticker in enumerate(tqdm(self.tickers)):
            self._collect_data_for_ticker(ticker, label)
